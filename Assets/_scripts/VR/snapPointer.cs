...
namespace VRTK
{
    ...
    public class snapPointer : VRTK_WorldPointer
    {
        ...
        protected override void Update()
        {
            ...
            if (pointer.gameObject.activeSelf)
            {
				...
				if (pointerCollidedWith.point != null) {
					Debug.Log (pointerCollidedWith.point);
					temp = FindClosestObject (pointerCollidedWith.point);
					current.GetComponent<MeshRenderer> ().material = red; // TODO: implementera active och inactive funktioner för gameobjects.
					current = temp;
					current.GetComponent<MeshRenderer> ().material = blue;
				}
            }
		...
		protected void Start() {
			current = FindClosestObject(new Vector3(0,0,0));
		...
		GameObject FindClosestObject(Vector3 point)
        {
            GameObject[] gos;
